#!/usr/bin/env python3
"""
Setup script for Network Security Analyzer
Spring 2024 Security Software Development

Installation script for the Python package.
"""

from setuptools import setup, find_packages
import os

# Read the README file
def read_readme():
    readme_path = os.path.join(os.path.dirname(__file__), 'README.md')
    if os.path.exists(readme_path):
        with open(readme_path, 'r', encoding='utf-8') as f:
            return f.read()
    return "Network Security Analyzer - Advanced network packet analysis and security monitoring"

# Read requirements
def read_requirements():
    requirements_path = os.path.join(os.path.dirname(__file__), 'python', 'requirements.txt')
    if os.path.exists(requirements_path):
        with open(requirements_path, 'r', encoding='utf-8') as f:
            return [line.strip() for line in f if line.strip() and not line.startswith('#')]
    return []

setup(
    name="network-security-analyzer",
    version="1.0.0",
    author="Network Security Analyzer Team",
    author_email="security@example.com",
    description="Advanced network packet analysis and security monitoring tool",
    long_description=read_readme(),
    long_description_content_type="text/markdown",
    url="https://github.com/sarrrthakkk/network-security-analyzer",
    project_urls={
        "Bug Tracker": "https://github.com/sarrrthakkk/network-security-analyzer/issues",
        "Documentation": "https://github.com/sarrrthakkk/network-security-analyzer/docs",
        "Source Code": "https://github.com/sarrrthakkk/network-security-analyzer",
    },
    classifiers=[
        "Development Status :: 4 - Beta",
        "Intended Audience :: Information Technology",
        "Intended Audience :: System Administrators",
        "Intended Audience :: Security Analysts",
        "License :: OSI Approved :: MIT License",
        "Operating System :: OS Independent",
        "Programming Language :: Python :: 3",
        "Programming Language :: Python :: 3.8",
        "Programming Language :: Python :: 3.9",
        "Programming Language :: Python :: 3.10",
        "Programming Language :: Python :: 3.11",
        "Programming Language :: Python :: 3.12",
        "Topic :: System :: Networking :: Monitoring",
        "Topic :: Security",
        "Topic :: System :: Systems Administration",
        "Topic :: Software Development :: Libraries :: Python Modules",
    ],
    keywords="network security analysis packet inspection anomaly detection threat detection",
    packages=find_packages(where="python"),
    package_dir={"": "python"},
    python_requires=">=3.8",
    install_requires=read_requirements(),
    extras_require={
        "dev": [
            "pytest>=6.0",
            "pytest-cov>=2.0",
            "black>=21.0",
            "flake8>=3.8",
            "mypy>=0.800",
            "sphinx>=4.0",
            "sphinx-rtd-theme>=0.5",
        ],
        "full": [
            "matplotlib>=3.3",
            "seaborn>=0.11",
            "plotly>=5.0",
            "flask>=2.0",
            "flask-cors>=3.0",
            "jinja2>=3.0",
            "redis>=4.0",
            "pymongo>=4.0",
            "cryptography>=3.4",
            "pycryptodome>=3.10",
            "yara-python>=4.0",
            "pefile>=2023.2",
            "virustotal-api>=1.1",
        ],
    },
    entry_points={
        "console_scripts": [
            "network-analyzer=src.network_analyzer:main",
        ],
    },
    include_package_data=True,
    package_data={
        "": ["*.md", "*.txt", "*.yml", "*.yaml"],
    },
    zip_safe=False,
    platforms=["any"],
    license="MIT",
    maintainer="Network Security Analyzer Team",
    maintainer_email="security@example.com",
    provides=["network_security_analyzer"],
    requires_python=">=3.8",
    test_suite="tests",
    tests_require=[
        "pytest>=6.0",
        "pytest-cov>=2.0",
    ],
    options={
        "bdist_wheel": {
            "universal": True,
        },
    },
)

